...
def reversalPhoto(photoAry, height, width):  # 2차원 사진 배열, 사진 배열의 행 개수, 사진 배열의 열 개수
    ...
    retAry = [[0 for _ in range(width)] for _ in range(height)]
    for i in range(height):
        for j in range(width):
            retAry[i][j] = photoAry[i][width - j - 1]
...
def zoomScale(photoAry, height, width):  # 2차원 사진 배열, 사진 배열의 행 개수, 사진 배열의 열 개수
    ...
    retAry = [[0 for _ in range(width * 2)] for _ in range(height * 2)]
    for i in range(height * 2):
        for j in range(width * 2):
            retAry[i][j] = photoAry[i // 2][j // 2]
...
def rotatePhoto(photoAry, height, width):  # 2차원 사진 배열, 사진 배열의 행 개수, 사진 배열의 열 개수
    ...
    retAry = [[0 for _ in range(height)] for _ in range(width)]
    print(retAry)
    for i in range(width):
        for j in range(height):
            retAry[i][j] = photoAry[j][width - i - 1]
